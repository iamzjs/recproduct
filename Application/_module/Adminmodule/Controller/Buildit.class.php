...
<load href="__PUBLIC__/ueditor143/ueditor.all.min.js"/>';
		  ...
		  foreach($params as $k=>$param){			  
			  ...
			  if(stristr($name,'xllb')){
				  ...
				  $colsnum++;
				  if($p[0]=='techno'){
					  $html.='<select class="form-control" name="'.$p[2].'" id="'.$p[2].'"><empty name="one.'.$p[2].'"><option value=""></option><else/><option value="{$one.'.$p[2].'}">{$one.'.$p[4].'}</option></empty>{$Think.config.TLIST}</select>';
					  $indexsql[] ='update '.$tbl.' t left join '.$p[0].' t1 on t.'.$p[2].'=t1.'.$id1.' set t.'.$p[4].'=t1.'.$name1;
				  }
				  elseif($p[0]=='course'){
					  $html.='<select class="form-control" name="'.$p[2].'" id="'.$p[2].'"><empty name="one.'.$p[2].'"><option value=""></option><else/><option value="{$one.'.$p[2].'}">{$one.'.$p[4].'}</option></empty>{$Think.config.CLIST}</select>';
					  $indexsql[] ='update '.$tbl.' t left join '.$p[0].' t1 on t.'.$p[2].'=t1.'.$id1.' set t.'.$p[4].'=t1.'.$name1;
				  }
				  else{
			  ...
			  elseif(stristr($name,'thumb')||stristr($name,'addr')||stristr($name,'attach')||stristr($name,'src')||stristr($name,'done')){
			  ...
			  $html.='</div></div>';
			  if(count($p)==6){
				$thview .= '<tr><th width="100" style="text-align:right">'.$p[3].':</th><td>{$one["'.$p[2].'"]}</td></tr><tr><th width="100" style="text-align:right">'.$p[5].':</th><td>{$one["'.$p[4].'"]}</td></tr>';
			  }
			  else{				  
				$thview .= '<tr><th width="100" style="text-align:right">'.$p[1].':</th><td>{$one["'.$name.'"]}</td></tr>';
			  }
		  ...
		  if(!empty($tbls)){
			  ...
			  foreach($tbls as $tt){
				  ...
				  if($tempparam[0]=='special')
					$formstr.='$this->assign("'.$tempparam[0].'list",M("'.$tempparam[0].'")->where(array(\'userid\'=>session("admin.id"),\'moduleid\'=>$this->moduleid))->select());';  
				  elseif($tempparam[0]!='course'&&$tempparam[0]!='techno')
		...
		file_put_contents($folder.$ctrl.'/thview.html',$thview);
		...
		foreach($params as $k=>$param){
			...
			elseif(stristr($p[0],'thumb')||stristr($p[0],'addr')||stristr($p[0],'attach')||stristr($p[0],'src')||stristr($p[0],'done')){
				//如果字段类型是文件上传相关，自动添加文件大小字段
				$sql.=','.$p[0].' '.$paramstype[$k];
				$sql.=','.$p[0].'size varchar(30)';
			}
			else{
				$sql.=','.$p[0].' '.$paramstype[$k];
			}
	...
	$sql .=',userid int not null default 1,hits int not null default 0,insert_time datetime,update_time timestamp not null default CURRENT_TIMESTAMP,check_userid int default 1,checktime datetime,checkstate int not null default 1,listorder int not null default 1,vstate int not null default 1,primary key(id))';
	M()->query($sql);
